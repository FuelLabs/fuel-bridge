mod success {
    ...
    async fn check_total_supply() {
        ...
        assert_eq!(
            total_supply(&implementation_contractid, &proxy_contract, asset_id).await.unwrap(),
    ...
    async fn check_total_assets() {
        ...
        assert_eq!(total_assets(&implementation_contractid, &proxy_contract).await, 1);
    }
    ...
    async fn check_name() {
        ...
        let response = name(&implementation_contractid, &proxy_contract, asset_id).await.unwrap();
    ...
    async fn check_symbol() {
        ...
        let response = symbol(&implementation_contractid, &proxy_contract, asset_id).await.unwrap();
    ...
    async fn check_decimals() {
        ...
        let response = decimals(&implementation_contractid, &proxy_contract, asset_id).await.unwrap();
