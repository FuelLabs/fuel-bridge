...
task("verify-deployment", "Verifies the deployed contract bytecode")
  .setAction(async (taskArgs: any, hre: HardhatRuntimeEnvironment): Promise<void> => {
    ...
    const deployments: Deployment[] = JSON.parse(fs.readFileSync(deploymentsFile, "utf8"));
    ...
    for (const deployment of deployments) {
      ...
      try {
        ...
        const artifactPath = path.join(`artifacts/contracts/${deployment.contractName}.sol/${deployment.contractName}.json`);
        ...
        if (deployment.isProxy) {
          ...
          await localHardhat.upgrades.validateUpgrade(deployment.proxyAddress as string, ContractFactory, { kind: "uups" });

          console.log("--- Upgrade success");
          localAddress = deployment.address;
        } else {
          const localContract = await ContractFactory.deploy(...deployment.args);
          await localContract.deployed();
          localAddress = await localContract.getAddress();
        }
