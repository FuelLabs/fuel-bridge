...
export type FuelMessagePortalFixture = {
  signers: HardhatEthersSigner[];
  fuelMessagePortal: FuelMessagePortalV3;
...
export function behavesLikeFuelMessagePortalV3(
  fixture: () => Promise<FuelMessagePortalFixture>
) {
  ...
  async function setupMessages(
    ...
    provider.send('evm_increaseTime', [TIME_TO_FINALIZE]);
  ...
  before('setup invariants', async () => {
    const fixt = await fixture();

    ({ fuelMessagePortal, fuelChainState, messageTester, signers } = fixt);

    addresses = fixt.signers.map((signer) => signer.address);
  ...
  describe('Behaves like FuelMessagePortalV3 - Relay both valid and invalid messages', () => {
    before(async () => {
      await fixture();
      await setupMessages(
        hre.ethers.provider,
  ...
  describe('Behaves like FuelMessagePortalV2 - Accounting', () => {
    beforeEach('fixture', async () => {
      await fixture();
      await setupMessages(
        hre.ethers.provider,
