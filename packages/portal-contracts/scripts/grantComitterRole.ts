...
const main = async () => {
  ...
  if (!FUEL_CHAIN_STATE_ADDRESS || !isAddress(FUEL_CHAIN_STATE_ADDRESS)) {
    throw new Error(
      `Invalid env var FUEL_CHAIN_STATE_ADDRESS ${FUEL_CHAIN_STATE_ADDRESS}`
    );
  }

  if (!COMITTER_ADDRESS || !isAddress(COMITTER_ADDRESS)) {
    throw new Error(
      `Invalid env var COMITTER_ADDRESS ${FUEL_CHAIN_STATE_ADDRESS}`
    );
  }
  ...
  const target = FuelChainState__factory.connect(
    FUEL_CHAIN_STATE_ADDRESS,
    admin
  );
