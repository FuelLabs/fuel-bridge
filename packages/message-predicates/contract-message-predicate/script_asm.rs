...
pub fn bytecode() -> Vec<u8> {
    let mut fn_selector_bytes = fuels::core::codec::encode_fn_selector("process_message");
    ...
    const REG_DATA_CONTRACT_PTR: u8 = 0x14;
    const REG_DATA_FN_SELECTOR_PTR: u8 = 0x15;
    const REG_DATA_CALLDATA_PTR: u8 = 0x16;
    const REG_CALLDATA_PTR: u8 = 0x17;
    
    //referenced data start pointer
    const REF_DATA_START_PTR: u16 = 14 * BYTES_PER_INSTR;

    let mut script: Vec<u8> = vec![
        op::move_(REG_MEMORY_START_PTR, RegId::SP), //REG_MEMORY_START_PTR = stack pointer
        op::cfei(320),
        op::addi(REG_DATA_PTR, REG_MEMORY_START_PTR, 32),
        ...
        op::addi(REG_DATA_CONTRACT_PTR, REG_DATA_PTR, 8),
        op::mcpi(REG_DATA_PTR, REG_CONTRACT_ADDR_PTR, 32),

        op::addi(REG_FN_SELECTOR_PTR, RegId::IS, REF_DATA_START_PTR),
        op::addi(REG_DATA_FN_SELECTOR_PTR, REG_DATA_PTR, 32),
        op::sw(REG_DATA_FN_SELECTOR_PTR, REG_FN_SELECTOR_PTR, 0),

        op::addi(REG_CALLDATA_PTR, RegId::IS, REF_DATA_START_PTR + 23),
        op::addi(REG_DATA_CALLDATA_PTR, REG_DATA_PTR, 40),
        op::sw(REG_DATA_CALLDATA_PTR, REG_CALLDATA_PTR, 0),
    ...
    script.append(&mut fn_selector_bytes);
    script.append(&mut 0u64.to_be_bytes().to_vec());
